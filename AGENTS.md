...
- `正常系统端/src/native_ui/theme.rs` 的单行 Edit 现行职责以本节“最终发布约束补充”为准：正常成功路径在完整 BGRA DIB 中先填充字段背景，再调用原生 `WM_PRINTCLIENT` 生成与真实 Edit 视口一致的正文、选区和水平滚动状态；默认过程先更新原生状态，主题层随后修复整帧 Alpha 并一次发布。不得把字段拆分成非客户区带，也不得用固定起点的 `DrawText` 替代原生 Edit 视口。DIB 创建失败时才允许直接向目标 DC 降级绘制；`WS_EX_LAYERED`、颜色键和逐字符 `WM_SETREDRAW` 仍然禁止。
