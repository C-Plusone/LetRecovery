...
- `PE端/src/native_ui/layout.rs`：PE P5 不依赖 HWND 的纯窗口几何，计算壳、进度、详情命令栏、进度条右侧同排百分比、monitor work-area 居中和夹取；宽高足够且全部步骤可见时，把标题、两组进度和使用 26px 逻辑行距的完整步骤列表作为一个略向上偏置的整体分配可用高度，不允许固定贴顶或把未使用高度全部堆在列表下方；纯进度页不得为已删除的底部状态文案保留空命令栏，详情页仍保留真实按钮所需命令栏；除 440×480、480×500 外还以低分辨率高 DPI 保守客户端区域覆盖 96–192 DPI，保证标题、当前步骤、总体进度、步骤列表和命令栏不相交；完整步骤空间不足时由调用方进入最多两列或只保留当前步骤文字与总体进度的 compact 展示。
- `PE端/src/native_ui/progress.rs`：PE P3/P4/P5 安装、备份和无损扩容的原生 Win32 长任务页，复用共享 `WorkflowSession` 单次启动 worker，以 50ms 定时器合并既有消息并只更新变化的文字/进度区域；加载圈另用约 16ms 的局部动画时钟并只覆盖步骤列表中真实 `InProgress` 行的状态图元，不得放在顶部当前步骤标题旁或退化为 50ms 工作轮询动画；动态安装/备份步骤保留无分配的静态键，仅在当前步骤或列表实际重绘时本地化，英语模式不得残留中文且轮询不得为翻译反复分配；首个真实步骤消息前显示准备态且不伪造当前步骤，顶部以 16px 逻辑标题字体显示助手名称，并只显示 `当前步骤：[名称]`，不再显示冗余说明；步骤文字必须使用 `SS_CENTERIMAGE` 在 26px 逻辑行内垂直居中，使其与同样按行中心定位的 16px 状态图元对齐；无损扩容的总进度直接跟随既有百分比；纯进度页不创建只读状态 Edit、不显示高级选项或关闭按钮，也不创建“操作进行中”“正在完成清理和收尾操作”等底部状态文案或空状态栏，详细诊断写入日志；当前步骤和总体百分比固定在各自进度条右侧同排；默认窗口使用 480×500 逻辑尺寸和 440×480 逻辑最小尺寸，初始窗口通过当前 HWND 最近显示器的工作区居中，`WM_DPICHANGED` 建议矩形仍夹到同一工作区；窗口类不设置图标且顶层窗口使用无标题栏图标的对话框框架扩展样式；`non-elevated-tests` 额外提供失败状态视觉入口以验收叉号，不启动 worker 或访问磁盘；消息循环异常仍等待同一 worker 真正结束；检查点、完成重启和底层错误语义保持不变。
