...
- 所有 report ListView 必须启用 `LVS_EX_DOUBLEBUFFER` 与 `WS_CLIPCHILDREN`；Header 禁用库存主题动画并由单一确定性入口绘制，Header 绘制不得反向失效整表。`WM_HSCROLL`、`WM_VSCROLL` 和滚轮必须先完成 comctl32 默认处理，再通过 ListView HWND 上唯一的待处理属性与约 16ms 单次定时器合并连续 `SB_THUMBTRACK`；定时器只调用一次普通 `InvalidateRect`，由 USER32 和 `LVS_EX_DOUBLEBUFFER` 完成合并绘制并清除原生像素位移复制出的外框残线。高频分支不得逐消息失效整表、调用 `UpdateWindow`、同步补画 Header/外框或直接竞争重定向表面。UxTheme 滚动条定时器仍只调用默认过程；唯一外框由正常 `WM_PAINT/WM_NCPAINT` 路径恢复。深色滚动条保留 comctl32 v6 原生语义并使用 `DarkMode_Explorer`，不得使用只支持旧 comctl32 的 FlatSB 颜色覆盖。
